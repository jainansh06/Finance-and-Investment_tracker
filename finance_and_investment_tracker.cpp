...
enum class ExpenseCategory { FOOD, TRANSPORT, UTILITIES, ENTERTAINMENT, HEALTHCARE, EDUCATION, OTHER };
...
public:
    ...
    string serialize() const { return to_string(day) + "," + to_string(month) + "," + to_string(year); }
...
public:
    Transaction(const string &desc, double amt, TransactionType t, ExpenseCategory cat = ExpenseCategory::OTHER)
        : id(nextId++), description(desc), amount(amt), date(Date()), type(t), category(cat) {}
    
    Transaction(const string &desc, double amt, const Date &d, TransactionType t, ExpenseCategory cat = ExpenseCategory::OTHER)
        : id(nextId++), description(desc), amount(amt), date(d), type(t), category(cat) {}
    ...
    string categoryToString() const {
        const char* categories[] = {"Food", "Transport", "Utilities", "Entertainment", "Healthcare", "Education", "Other"};
        return categories[static_cast<int>(category)];
    }

    void display() const {
        cout << left << setw(5) << id << setw(20) << description
                  << setw(12) << fixed << setprecision(2) << "₹" << amount
                  << setw(12) << typeToString() << setw(15) << categoryToString()
                  << setw(12) << date.toString() << endl;
    }
    ...
    static Transaction deserialize(const string &str) {
        ...
        getline(ss, token, ','); string desc = token;
        getline(ss, token, ','); double amount = stod(token);
        ...
        getline(ss, token, ','); TransactionType type = static_cast<TransactionType>(stoi(token));
        getline(ss, token, ','); ExpenseCategory category = static_cast<ExpenseCategory>(stoi(token));
...
public:
    Investment(const string &sym, const string &n, InvestmentType t, double qty, double price, const Date &date = Date())
        : symbol(sym), name(n), type(t), quantity(qty), purchasePrice(price), currentPrice(price), purchaseDate(date) {}
    ...
    double getGainLossPercentage() const { return (getGainLoss() / getInitialValue()) * 100.0; }
    ...
    void display() const {
        cout << left << setw(8) << symbol << setw(20) << name
                  << setw(12) << typeToString() << setw(10) << fixed << setprecision(2) << quantity
                  << setw(12) << "₹" << purchasePrice << setw(12) << "₹" << currentPrice
                  << setw(12) << "₹" << getCurrentValue() << setw(12) << "₹" << getGainLoss()
                  << setw(10) << setprecision(1) << getGainLossPercentage() << "%" << endl;
    }
    ...
    static Investment deserialize(const string &str) {
        stringstream ss(str);
        string token;
        getline(ss, token, ','); string symbol = token;
        getline(ss, token, ','); string name = token;
        getline(ss, token, ','); InvestmentType type = static_cast<InvestmentType>(stoi(token));
        getline(ss, token, ','); double quantity = stod(token);
        getline(ss, token, ','); double purchasePrice = stod(token);
        getline(ss, token, ','); double currentPrice = stod(token);
...
public:
    ...
    double getTotalGainLossPercentage() const {
        double totalInitial = accumulate(investments.begin(), investments.end(), 0.0,
            [](double sum, const unique_ptr<Investment> &inv) {
    ...
    void display() const {
        cout << "\n=== Portfolio: " << name << " ===" << endl;
        cout << left << setw(8) << "Symbol" << setw(20) << "Name" << setw(12) << "Type"
                  << setw(10) << "Quantity" << setw(12) << "Buy Price" << setw(12) << "Current"
                  << setw(12) << "Value" << setw(12) << "Gain/Loss" << setw(10) << "%" << endl;
        ...
        cout << string(120, '-') << endl;
        cout << "Total Portfolio Value: ₹" << fixed << setprecision(2) << getTotalValue() << endl;
        cout << "Total Gain/Loss: ₹" << getTotalGainLoss() << " (" << setprecision(1) << getTotalGainLossPercentage() << "%)" << endl;
    }
...
public:
    FinanceTracker(const string &dataFileName = "finance_data.csv", const string &portfolioFileName = "portfolio_data.csv")
    ...
    void addTransaction(const string &description, double amount, TransactionType type, ExpenseCategory category = ExpenseCategory::OTHER) {
        transactions.push_back(make_unique<Transaction>(description, amount, type, category));
    }

    void addInvestment(const string &symbol, const string &name, InvestmentType type, double quantity, double price) {
        auto investment = make_unique<Investment>(symbol, name, type, quantity, price);
        portfolio->addInvestment(move(investment));
        addTransaction("Investment: " + symbol, quantity * price, TransactionType::INVESTMENT);
    }

    double getTotalIncome() const {
        return accumulate(transactions.begin(), transactions.end(), 0.0,
            [](double sum, const unique_ptr<Transaction> &t) {
                return sum + (t->getType() == TransactionType::INCOME ? t->getAmount() : 0);
            });
    }
    ...
    double getTotalExpenses() const {
        return accumulate(transactions.begin(), transactions.end(), 0.0,
            [](double sum, const unique_ptr<Transaction> &t) {
                return sum + (t->getType() == TransactionType::EXPENSE ? t->getAmount() : 0);
            });
    }
    ...
    double getTotalInvestments() const {
        return accumulate(transactions.begin(), transactions.end(), 0.0,
            [](double sum, const unique_ptr<Transaction> &t) {
                return sum + (t->getType() == TransactionType::INVESTMENT ? t->getAmount() : 0);
            });
    }
    ...
    void displayTransactions() const {
        ...
        cout << left << setw(5) << "ID" << setw(20) << "Description" << setw(12) << "Amount"
                  << setw(12) << "Type" << setw(15) << "Category" << setw(12) << "Date" << endl;
        cout << string(80, '-') << endl;
        for (const auto &t : transactions) {
            t->display();
        }
    ...
    void displaySummary() const {
        ...
        for (const auto &pair : expenseBreakdown) {
            cout << "  " << static_cast<int>(pair.first) << ": ₹" << pair.second << endl;
        }
    }

    void displayPortfolio() const { portfolio->display(); }
    void updateMarketPrices() { portfolio->updateMarketPrices(); cout << "Market prices updated!" << endl; }
    ...
    void run() {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 2: addInvestment(); break;
                case 3: tracker.displayTransactions(); break;
                case 4: tracker.displayPortfolio(); break;
                case 5: tracker.displaySummary(); break;
                case 6: tracker.updateMarketPrices(); break;
                case 7: cout << "Saving data and exiting..." << endl; tracker.saveData(); break;
...
private:
    void addTransaction() {
        ...
        if (type == TransactionType::EXPENSE) {
            cout << "Select category (1-Food, 2-Transport, 3-Utilities, 4-Entertainment, 5-Healthcare, 6-Education, 7-Other): ";
            cin >> categoryChoice;
